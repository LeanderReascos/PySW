...
def solver(H, order):
    ...
    for k in range(1, order+1):
        ...
        for term in S_k.terms:
            ...
            for op in term.info["finite"]:
                op.mat_representation = op.mat_representation.subs(solution_sk)
            if sum(sp.Abs(term.mat_representation["finite"])) == 0:
                continue
            S_k_solved += term
        S += S_k_solved
        print(k)
        tmp_H =  H_below_k.nested_commutator(S, k)
        print("hi")
        tmp_H *= float(1 / sp.factorial(k)) 
        print("hi")
...
please_god_may_this_work = solver(H, 2)